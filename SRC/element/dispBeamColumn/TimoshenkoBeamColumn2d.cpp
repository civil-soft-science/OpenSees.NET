 ...
 Q(6), q(3), rho(r), parameterID(0)
{
  ...
  if (theSections == 0) {
    opserr << "TimoshenkoBeamColumn2d::TimoshenkoBeamColumn2d - failed to allocate section model pointer\n";
    exit(-1);
  }
  ...
  for (int i = 0; i < numSections; i++) {
    ...
    // Check allocation
    if (theSections[i] == 0) {
      opserr << "TimoshenkoBeamColumn2d::TimoshenkoBeamColumn2d -- failed to get a copy of section model\n";
      exit(-1);
    }
  }
  ...
  if (beamInt == 0) {
    opserr << "TimoshenkoBeamColumn2d::TimoshenkoBeamColumn2d - failed to copy beam integration\n";
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "TimoshenkoBeamColumn2d::TimoshenkoBeamColumn2d - failed to copy coordinate transformation\n";
    exit(-1);
  }
...
TimoshenkoBeamColumn2d::setDomain(Domain *theDomain)
{
    ...
    if (theNodes[0] == 0 || theNodes[1] == 0) {
      opserr << "WARNING TimoshenkoBeamColumn2d (tag: %d), node not found in domain" << this->getTag() << endln;;
      return;
    }
	...
	//opserr << "FATAL ERROR TimoshenkoBeamColumn2d (tag: %d), has differing number of DOFs at its nodes",
	//	this->getTag());
...
TimoshenkoBeamColumn2d::update(void)
{
  ...
  if (err != 0) {
    opserr << "TimoshenkoBeamColumn2d::update() - failed setTrialSectionDeformations()\n";
    return err;
  }
...
TimoshenkoBeamColumn2d::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  ...
  if (type == LOAD_TAG_Beam2dUniformLoad) {
    ...
    opserr << "TimoshenkoBeamColumn2d::TimoshenkoBeamColumn2d -- load type unknown for element with tag: "
...
TimoshenkoBeamColumn2d::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  for (j = 0; j<numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "TimoshenkoBeamColumn2d::sendSelf() - section " << 
			   ...
			   FEM_ObjectBroker &theBroker)
{
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
      ...
      if (crdTransf == 0) {
	opserr << "TimoshenkoBeamColumn2d::recvSelf() - failed to obtain a CrdTrans object with classTag " <<
  ...
  if (beamInt == 0 || beamInt->getClassTag() != beamIntClassTag) {
      ...
      if (beamInt == 0) {
	opserr << "TimoshenkoBeamColumn2d::recvSelf() - failed to obtain the beam integration object with classTag" <<
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
opserr << "TimoshenkoBeamColumn2d::recvSelf() - out of memory creating sections array of size " <<
  idData(3) << endln;
      return -1;
    }    
    ...
    for (i=0; i<numSections; i++) {
      ...
      if (theSections[i] == 0) {
	opserr << "TimoshenkoBeamColumn2d::recvSelf() - Broker could not create Section of class type " <<
	  sectClassTag << endln;
	exit(-1);
      }
      theSections[i]->setDbTag(sectDbTag);
      if (theSections[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
	opserr << "TimoshenkoBeamColumn2d::recvSelf() - section " << i << " failed to recv itself\n";
	return -1;
      }     
    }
...
Response*
TimoshenkoBeamColumn2d::setResponse(const char **argv, int argc,
			      OPS_Stream &output)
{
...
TimoshenkoBeamColumn2d::getResistingForceSensitivity(int gradNumber)
{
  ...
  for (int i = 0; i < numSections; i++) {
	...
	break;
      case SECTION_RESPONSE_MZ:
	dqdh(1) += 1.0/(1+phi)*(xi6-4.0-phi)*sensi - dphidh/pow((1+phi),2)*(xi6-4.0-phi)*si + 1.0/(1+phi)*(-dphidh)*si;
	dqdh(2) += 1.0/(1+phi)*(xi6-2.0+phi)*sensi - dphidh/pow((1+phi),2)*(xi6-2.0+phi)*si + 1.0/(1+phi)*(+dphidh)*si; 
	break;
      case SECTION_RESPONSE_VY:
	dqdh(1) += 0.5*phi*L/(1+phi)*sensi + 0.5*dphidh/pow((1+phi),2)*L*si + 0.5*phi*dLdh/(1+phi)*si;
	dqdh(2) += 0.5*phi*L/(1+phi)*sensi + 0.5*dphidh/pow((1+phi),2)*L*si + 0.5*phi*dLdh/(1+phi)*si;
	break;
      default:
	break;
      }
...
TimoshenkoBeamColumn2d::commitSensitivity(int gradNumber, int numGrads)
{
  ...
  for (int i = 0; i < numSections; i++) {
	...
	break;
      case SECTION_RESPONSE_MZ:
	e(j) = oneOverL*1.0/(1+phi)*((xi6-4.0-phi)*dvdh(1) + (xi6-2.0+phi)*dvdh(2))
	  + d1oLdh*1.0/(1+phi)*((xi6-4.0-phi)*v(1) + (xi6-4.0-phi)*v(2))
	  -oneOverL*dphidh/pow((1+phi),2)*((xi6-4.0-phi)*v(1) + (xi6-4.0-phi)*v(2))
