...
void* OPS_TripleFrictionPendulum()
{
	...
	{
		numData = 6;
		if (OPS_GetDoubleInput(numData, stpr_data) != 0) {
			opserr << "WARNING error reading stpr inelastic behavior data" << eleTag << endln;
			return 0;
		}
	}
	...
	Element* theTripleFrictionPendulum = new TripleFrictionPendulum(eleTag, iData[1], iData[2], theFrnMdls, theMaterials, dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], dData[9], dData[10],
		stpr_data[0], stpr_data[1], stpr_data[2], stpr_data[3], stpr_data[4], stpr_data[5]);
...
TripleFrictionPendulum::TripleFrictionPendulum(int tag,
	...
	double kvt,
	double minFv,
	double tol, double _fy, double _as, double _dp, double _dc, double _du, double _resFyFac)
	...
	stpr_fy(_fy), stpr_as(_as), stpr_dpFac(_dp), stpr_dcFac(_dc), stpr_duFac(_du), stpr_resFyFac(_resFyFac),
	loadDir(3), loadDirPrev(3), stpr_dCur(3), stpr_dPrev(3), stpr_fCur(3), stpr_fPrev(3), stpr_peakFCur(3), stpr_peakFPrev(3), plastGap(3), plastGapPrev(3), initGap(3)//, fj(2)
{
...
TripleFrictionPendulum::TripleFrictionPendulum()
	...
	ep1(2), ep1pr(2), ep3(2), ep3pr(2), ep5(2), ep5pr(2),
	q1(2), q1pr(2), q3(2), q3pr(2), q5(2), q5pr(2),
	ep1tmp(2), ep3tmp(2), ep5tmp(2), q1tmp(2), q3tmp(2), q5tmp(2), stpr_fy(0),
	loadDir(3), loadDirPrev(3), stpr_dCur(3), stpr_dPrev(3), stpr_peakFCur(3), stpr_peakFPrev(3), plastGap(3), plastGapPrev(3), initGap(3)//, fj(2)
{
...
int TripleFrictionPendulum::commitState()
{
	...
	for (int i = 0; i < 3; i++)
	{
		plastGapPrev(i) = plastGap(i);
		stpr_dPrev(i) = stpr_dCur(i);
		stpr_fPrev(i) = stpr_fCur(i);
		loadDirPrev(i) = loadDir(i);
		stpr_peakFPrev(i) = stpr_peakFCur(i);
	}
	return 0;
}
...
int TripleFrictionPendulum::revertToLastCommit()
{
	...
	for (int i = 0; i < 3; i++)
	{
		plastGap(i) = plastGapPrev(i);
		stpr_dCur(i) = stpr_dPrev(i);
		stpr_fCur(i) = stpr_fPrev(i);
		loadDir(i) = loadDirPrev(i);
		stpr_peakFCur(i) = stpr_peakFPrev(i);
	}
	return 0;
}
...
int TripleFrictionPendulum::revertToStart()
{
	...
	initGap(0) = Gap2; initGap(1) = Gap4; initGap(2) = Gap6;
	plastGap(0) = Gap2; plastGap(1) = Gap4; plastGap(2) = Gap6;
	plastGapPrev(0) = Gap2; plastGapPrev(1) = Gap4; plastGapPrev(2) = Gap6;
	loadDirPrev(0) = loadDir(0) = 0;
	loadDirPrev(1) = loadDir(1) = 0;
	loadDirPrev(2) = loadDir(2) = 0;
	stpr_dCur.Zero(); stpr_dPrev.Zero();
	stpr_fCur.Zero(); stpr_fPrev.Zero();
	stpr_peakFCur.Zero(); stpr_peakFPrev.Zero();
	return errCode;
}
...
Element* TripleFrictionPendulum::getCopy()
{
	TripleFrictionPendulum* theCopy = new TripleFrictionPendulum(this->getTag(),
		...
		Ubar1, Ubar2, Ubar3, W, Uy, Kvt, MinFv, TOL, stpr_fy, stpr_as, stpr_dpFac, stpr_dcFac, stpr_duFac, stpr_resFyFac);
	theCopy->Kpr = Kpr;
	theCopy->fpr = fpr;
	theCopy->k12pr = k12pr;
	theCopy->k34pr = k34pr;
	...
	theCopy->Fy3pr = Fy3pr;
	theCopy->Fy5pr = Fy5pr;
	theCopy->stpr_dPrev = stpr_dPrev;
	theCopy->stpr_dCur = stpr_dCur;
	theCopy->stpr_fPrev = stpr_fPrev;
	theCopy->stpr_fCur = stpr_fCur;
	theCopy->stpr_peakFCur = stpr_peakFCur;
	theCopy->stpr_peakFPrev = stpr_peakFPrev;
	theCopy->loadDir = loadDir;
	theCopy->loadDirPrev = loadDirPrev;
	theCopy->plastGapPrev = plastGapPrev;
	theCopy->plastGap = plastGap;
	theCopy->initGap = initGap;
	...
	OPS_Stream& output)
{
	...
	else if (strcmp(argv[0], "stopperDisp") == 0) {
		theResponse = new ElementResponse(this, 7, Vector(3));
	}
	else if (strcmp(argv[0], "stopperForce") == 0) {
...
int TripleFrictionPendulum::getResponse(int responseID, Information& eleInfo)
{
	...
	case 7:
		return eleInfo.setVector(stpr_dPrev);
	case 8:
		return eleInfo.setVector(stpr_fPrev);
	default:
		return -1;
	}
}

void TripleFrictionPendulum::CircularPlasticGap(Matrix& kj, Vector& fj, double Ej, double Gapj, Vector di, int iPend)
{
	//static std::ofstream strm1("pend1.txt"), strm2("pend2.txt"), strm3("pend3.txt");
	//std::ofstream& strm = iPend == 1 ? strm1 : (iPend == 2 ? strm2 : strm3);
	const double& gap0 = initGap(iPend - 1);
	double& gap = plastGap(iPend - 1);
	gap = plastGapPrev(iPend - 1);
	double& d = stpr_dCur(iPend - 1);
	double& dNormPrev = stpr_dPrev(iPend - 1);
	double& peakF = stpr_peakFCur(iPend - 1);
	peakF = stpr_peakFPrev(iPend - 1);
	double& f = stpr_fCur(iPend - 1);
	int& dir = loadDir(iPend - 1);
	int& dirPrev = loadDirPrev(iPend - 1);
	d = di.Norm();
	double r = d - gap;
	if (r <= 0) {
		f = 0;
		...
		if (d >= gap0)
			...
			gap = gap0;
		//strm << theDmin->getCurrentTime() << " " << 0 << " " << r << " " << gap << " " << d << endln;
		return;
	}
	if (d < dNormPrev)
		dir = -1;
	...
	if (dir == -1 && dirPrev == 1)
	{
		// we are unloading for the first time; reset the gap
		double dNormPlast = dNormPrev - peakF / Ej;
		if (dNormPlast < gap0)
			dNormPlast = gap0;
		gap = dNormPlast;
		r = d - gap;
	...
	if (dir > 0) {
		//compute stopper envelope
		double stpr_dy = stpr_fy / Ej;
		double stpr_dp = stpr_dpFac * stpr_dy;
		double stpr_dc = stpr_dcFac * stpr_dy;
		double stpr_du = stpr_duFac * stpr_dy;
		double stpr_ks = stpr_as * Ej;
		double stpr_fc = stpr_fy + (stpr_dp - stpr_dy) * stpr_ks;
		double stpr_fr = stpr_fy * stpr_resFyFac;
		double stpr_kc = (stpr_fr - stpr_fc) / (stpr_dc - stpr_dp);

		//compute the yield point of the truncated backbone governing the re-loading branch
		double bc_dy = stpr_dy + gap - gap0;    //gap-gap0 == plastic d
		double fy = 0;
		if (bc_dy <= stpr_dp) {
			fy = stpr_fy + (bc_dy - stpr_dy) * stpr_ks;
		}
		else if (bc_dy <= stpr_dc) {
			fy = stpr_fc + stpr_kc * (bc_dy - stpr_dp);
		}
		else {
			fy = stpr_fr;
		}
		r = d - gap0;
		//locate the current f-d point on the backbone
		if (r <= bc_dy) {
			Ej0 = Ej;
			f = Ej * (d - gap);
		}
			...
			Ej0 = stpr_ks;
			f = stpr_fy + stpr_ks * (r - stpr_dy);
		}
		else if (r <= stpr_dc) {
			Ej0 = stpr_kc;// 1e-3 * Ej;
			f = stpr_fc + stpr_kc * (r - stpr_dp);
		}
		else if (r <= stpr_du) {
			Ej0 = 0;// 1e-3 * Ej;
			f = stpr_fr;
	...
	double sn = di(1) / d;
	double cs = di(0) / d;
	kj(0, 0) = Ej0 * (1 - Gapj / r * sn * sn);
	kj(0, 1) = kj(1, 0) = Ej0 * Gapj / r * sn * cs;
	kj(1, 1) = Ej0 * (1 - Gapj / r * cs * cs);
	fj(0) = f * cs;
	fj(1) = f * sn;
	//strm << theDmin->getCurrentTime() << " " << f << " " << r << " " << gap << " " << d << endln;
	//strm.flush();
}
...
void TripleFrictionPendulum::Segment(int iPend, Vector& epitmp, Vector& qitmp, bool& conv, Matrix& kij, Vector& di, Vector epi, Vector qi, Vector f, Vector df, double Fyi, double Ei, double Hi, double Ej, double Gapj, double Tol, int Niter)
{
	Vector dftmp = df;
	Vector dd;
	Matrix ki(2, 2);
	...
	while (((dd.Norm() > 0.01 * Tol) && (iter <= Niter)) || (!enterloop))
	{
		...
		if (stpr_fy < 1e-6)
			...
			CircularPlasticGap(kj, fj, Ej, Gapj, di, iPend);
		kij = ki + kj;
		fprime = fi + fj;
		dftmp = f + df - fprime;
		kij.Invert(invkij);
...
void TripleFrictionPendulum::TFPElement(bool& Conv, Vector& ep1tmp, Vector& ep3tmp, Vector& ep5tmp, Vector& q1tmp, Vector& q3tmp, Vector& q5tmp, Matrix& K, Vector& f, Matrix& k12, Matrix& k34, Matrix& k56, Vector& d1, Vector& d3, Vector& d5, Vector ep1, Vector ep3, Vector ep5, Vector q1, Vector q3, Vector q5, Vector u, Vector dusub, double Fy1, double Fy3, double Fy5, double E1, double E3, double E5, double H1, double H3, double H5, double E2, double E4, double E6, double Gap2, double Gap4, double Gap6, double Tol, int Niter)
{
	...
	while (((du.Norm() > Tol) && (iter <= Niter) && Conv) || (!enterloop))
	{
		...
		iter++;
		df = K * du;
		Segment(1, ep1tmp, q1tmp, conv, k12, d1, ep1, q1, f, df, Fy1, E1, H1, E2, Gap2, Tol, Niter);
		if (!conv)
		{
			Conv = false;
			break;
		}
		Segment(2, ep3tmp, q3tmp, conv, k34, d3, ep3, q3, f, df, Fy3, E3, H3, E4, Gap4, Tol, Niter);
		if (!conv)
		{
			Conv = false;
			break;
		}
		Segment(3, ep5tmp, q5tmp, conv, k56, d5, ep5, q5, f, df, Fy5, E5, H5, E6, Gap6, Tol, Niter);
		if (!conv)
		{
			Conv = false;
			break;
