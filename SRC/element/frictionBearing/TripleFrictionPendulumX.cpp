...
// Version 2.0: TripleFrictionPendulumX element is capable of accounting for the effects of the sliding plate thickness on temperature calculation. 
// Detailed information on heat conduction solutions implemented in the element can be found in Kim and Constantinou (2024)
...
TripleFrictionPendulumX::TripleFrictionPendulumX(int tag,
    ...
    : Element(tag, ELE_TAG_TripleFrictionPendulumX), externalNodes(2), tag1(tg), tag2(tg2), kpFactor(Pfactor), kTFactor(Tfactor), kvFactor(Vfactor),
    Mu_ref1(muref1), Mu_ref2(muref2), Mu_ref3(muref3), L1(l1), L2(l2), L3(l3), Ubar1(ubar1), Ubar2(ubar2), Ubar3(ubar3), B1(b1), B2(b2), B3(b3), PLATE2(plate2), PLATE3(plate3),
...
TripleFrictionPendulumX::TripleFrictionPendulumX()
    : Element(0, ELE_TAG_TripleFrictionPendulumX), externalNodes(2), tag1(0), tag2(0), kpFactor(0), kTFactor(0), kvFactor(0),
    Mu_ref1(0.0), Mu_ref2(0.0), Mu_ref3(0.0), L1(0.0), L2(0.0), L3(0.0), Ubar1(0.0), Ubar2(0.0), Ubar3(0.0), B1(0.0), B2(0.0), B3(0.0), PLATE2(0.0), PLATE3(0.0),
...
int TripleFrictionPendulumX::revertToStart()
{
    ...
    Temperature_Depth2 = Temperature_Depth3 = Temperature0;
...
int TripleFrictionPendulumX::update()
{
    ...
    if (trialT > DomainTime(iCountTime - 1)) {
        ...
        // Temperature at depth (only for two outer surfaces) //
        Temperature_Depth_Change2 = 0.0;
        Temperature_Depth_Change3 = 0.0;

        dTdt2_surface = 0.0; dTdt2_depth = 0.0; 
        dTdt3_surface = 0.0; dTdt3_depth = 0.0;

        // Series element 1
        if (iCountTime > 1 && trialDisp1 > 0.00000000001) {
            DtAnalysis = 0.000001;
            tau = 0.000001;
        ...
        // Series element 2
        if (iCountTime > 1 && trialDisp2 > 0.00000000001) {
            DtAnalysis = 0.000001;
            tau = 0.000001;
            for (int jTemp = 0; jTemp <= iCountTime; jTemp++) {
                if (iCountTime == 0) {
                ...
                if (tag2 == 1) {
                    // Theory for indefinite half space and indefinite heat source at the surface
                    ...
                    Temperature_Depth_Change2 = Temperature_Depth_Change2 + sqrt(Diffusivity * 7.0 / 22.0) * DomainHeatFlux2(iCountTime - jTemp) * DtAnalysis / (Conductivity * sqrt(tau)) * exp(-PLATE2 * PLATE2 / (4 * Diffusivity * tau)); 

                }
                else {
                    // Theory of heated plate of finite depth over insulated space and indefinite heat source at the surface
                    dTdt2_surface = dTdt_FINITE(Diffusivity, Conductivity, PLATE2, 0, tau);
                    Temperature_Change2 = Temperature_Change2 + DomainHeatFlux2(iCountTime - jTemp) * dTdt2_surface * DtAnalysis;

                    dTdt2_depth = dTdt_FINITE(Diffusivity, Conductivity, PLATE2, PLATE2, tau);
                    Temperature_Depth_Change2 = Temperature_Depth_Change2 + DomainHeatFlux2(iCountTime - jTemp) * dTdt2_depth * DtAnalysis;    
                }
        ...
        Temperature_Depth2 = Temperature0 + Temperature_Depth_Change2;
        TemperatureDepth2(0) = Temperature_Depth2;

        // Series element 3
        if (iCountTime > 1 && trialDisp3 > 0.00000000001) {
            DtAnalysis = 0.000001;
            tau = 0.000001;
            for (int jTemp = 0; jTemp <= iCountTime; jTemp++) {
                if (iCountTime == 0) {
                ...
                if (tag2 == 1) {
                    ...
                    Temperature_Depth_Change3 = Temperature_Depth_Change3 + sqrt(Diffusivity * 7.0 / 22.0) * DomainHeatFlux3(iCountTime - jTemp) * DtAnalysis / (Conductivity * sqrt(tau)) * exp(-PLATE3 * PLATE3 / (4 * Diffusivity * tau));

                }
                else {
                    // Theory of heated plate of finite depth over insulated space and indefinite heat source at the surface
                    dTdt3_surface = dTdt_FINITE(Diffusivity, Conductivity, PLATE3, 0, tau);
                    Temperature_Change3 = Temperature_Change3 + DomainHeatFlux3(iCountTime - jTemp) * dTdt3_surface * DtAnalysis;

                    dTdt3_depth = dTdt_FINITE(Diffusivity, Conductivity, PLATE3, PLATE3, tau);
                    Temperature_Depth_Change3 = Temperature_Depth_Change3 + DomainHeatFlux3(iCountTime - jTemp) * dTdt3_depth * DtAnalysis;
                }
        ...
        Temperature_Depth3 = Temperature0 + Temperature_Depth_Change3;
        TemperatureDepth3(0) = Temperature_Depth3;
...
Element* TripleFrictionPendulumX::getCopy()
{
    TripleFrictionPendulumX* theCopy = new TripleFrictionPendulumX(this->getTag(),
        ...
        Ubar1, Ubar2, Ubar3, B1, B2, B3, PLATE2, PLATE3, W, Uy, Kvt, MinFv, TOL, refPressure1, refPressure2, refPressure3, Diffusivity, Conductivity, Temperature0, rateParam, tempParam, unit); //, rContact, kTFactor, diffuse, conduct
    theCopy->Kpr = Kpr;
    theCopy->fpr = fpr;
    theCopy->k12pr = k12pr;
    theCopy->k34pr = k34pr;
...
int TripleFrictionPendulumX::sendSelf(int commitTag, Channel& theChannel)
{
    ...
    data(23) = MinFv;
    data(24) = TOL;
    data(25) = refPressure1;
    data(26) = refPressure2;
    data(27) = refPressure3;
    data(28) = Diffusivity;
    data(29) = Conductivity;
    data(30) = Temperature0;
    data(31) = rateParam;
    data(32) = tempParam;
    data(33) = unit;
...
int TripleFrictionPendulumX::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
    ...
    MinFv = data(23);
    TOL = data(24);
    refPressure1 = data(25);
    refPressure2 = data(26);
    refPressure3 = data(27);
    Diffusivity = data(28);
    Conductivity = data(29);
    Temperature0 = data(30);
    rateParam = data(31);
    tempParam = data(32);
    unit = data(33);
...
void TripleFrictionPendulumX::Print(OPS_Stream& s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        ...
        s << "  PLATE2: " << PLATE2 << ", PLATE3: " << PLATE3 << endln;
    ...
    OPS_Stream& output)
{
        ...
        strcmp(argv[0], "parameters") == 0 || strcmp(argv[0], "Parameters") == 0)
    {
        ...
        output.tag("ResponseType", "Temp1 at depth");
        output.tag("ResponseType", "Temp4 at depth");
        ...
        theResponse = new ElementResponse(this, 7, Vector(20));
    }
...
{
    Vector locForce(12), locDisp(12), basForce(6), basDisp(6), cmpDisp(12), Param(20), Surf(16);
    switch (responseID) {
    ...
    case 7:  // Temperature on each sliding surface
        ...
        Param(3) = TemperatureDepth2(0); 
        Param(4) = TemperatureDepth3(0); 
        Param(5) = Mu_Adj1;
        Param(6) = Mu_Adj2;
        Param(7) = Mu_Adj3;
        Param(8) = HeatFluxCenter1(0);
        Param(9) = HeatFluxCenter2(0);
        Param(10) = HeatFluxCenter3(0);
        Param(11) = kpF1;
        Param(12) = kpF2;
        Param(13) = kpF3;
        Param(14) = kTF1;
...
{                                                   // k12pr(34,56)    tmp1         tmp2          tmp3           Fy1(3,5)    E1(3,5)    H1(3,5)    ep1pr(3,5)  q1pr(3,5)  d1pr(3,5)           
...
double TripleFrictionPendulumX::dTdt_FINITE(double Diffusivity, double Conductivity, double SlabLength, double depthz, double tau)
{
    int NNN = 0;
    double dTdt1 = 0;
    double dTdt1pr = 0;
    ...
    while (NNN <= 100) {
        dTdt1 += sqrt(Diffusivity) / (Conductivity * sqrt(tau)) * ((1 / sqrt(PI) * exp(-(2 * NNN * SlabLength + depthz) / (2 * sqrt(Diffusivity * tau)) * (2 * NNN * SlabLength + depthz) / (2 * sqrt(Diffusivity * tau))) - (2 * NNN * SlabLength + depthz) / (2 * sqrt(Diffusivity * tau)) * erfc((2 * NNN * SlabLength + depthz) / (2 * sqrt(Diffusivity * tau)))) + 1 / sqrt(PI) * exp(-(2 * (NNN + 1) * SlabLength - depthz) / (2 * sqrt(Diffusivity * tau)) * (2 * (NNN + 1) * SlabLength - depthz) / (2 * sqrt(Diffusivity * tau))) - ((2 * (NNN + 1) * SlabLength - depthz) / (2 * sqrt(Diffusivity * tau))) * erfc((2 * (NNN + 1) * SlabLength - depthz) / (2 * sqrt(Diffusivity * tau)))) + 2 * sqrt(Diffusivity * tau) / Conductivity * (erfc((2 * NNN * SlabLength + depthz) / (2 * sqrt(Diffusivity * tau))) * (2 * NNN * SlabLength + depthz) / (4 * sqrt(Diffusivity)) * pow(tau, -1.5) + erfc((2 * (NNN + 1) * SlabLength - depthz) / (2 * sqrt(Diffusivity * tau))) * (2 * (NNN + 1) * SlabLength - depthz) / (4 * sqrt(Diffusivity)) * pow(tau, -1.5));
        if ((dTdt1 - dTdt1pr) / (dTdt1pr) * 100 > 0.1) {
            NNN++;
            dTdt1pr = dTdt1;
        }
        else {
