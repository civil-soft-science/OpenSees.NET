...
int  ShellNLDKGT::sendSelf (int commitTag, Channel &theChannel)
{
  ...
  if (theDamping[0]) {
    for (int i = 0 ;  i < 4; i++) {
      ...
      if (res < 0) {
        opserr << "ShellNLDKGT::sendSelf -- could not send Damping\n";
        return res;
      }
    }
  }
  ...
  if (dmpTag) {
    for (i = 0 ;  i < 4; i++) {
      ...
      if (theDamping[i] == 0) {
        ...
        if (theDamping[i] == 0) {
          opserr << "ShellNLDKGT::recvSelf -- could not get a Damping\n";
          exit(-1);
        }
      }
      ...
      if (res < 0) {
        opserr << "ShellNLDKGT::recvSelf -- could not receive Damping\n";
        return res;
      }
    }
  }
