...
// $Source: /usr/local/cvs/OpenSees/SRC/element/zeroLength/ZeroLengthVG_HG.cpp,v $

// Written: GLF
// Created: 12/99
// Revision: A
//
// Description: This file contains the implementation for the ZeroLengthVG_HG class.
//
// What: "@(#) ZeroLengthVG_HG.C, revA"

#include "ZeroLengthVG_HG.h"
...
OPS_ZeroLengthVG_HG(void)
{
    ...
    if (numdata < 7) {
        opserr << "WARNING too few arguments " <<
            "want - element ZeroLengthVG_HG eleTag? iNode? jNode? " <<
    ...
    if (strcmp(type,"-mat") != 0) {
        opserr << "WARNING expecting " <<
            "- element ZeroLengthVG_HG eleTag? iNode? jNode? " <<
            "-mat matID1? ... -dir dirMat1? .. " <<
            "<-orient x1? x2? x3? y1? y2? y3?>\n";
    ...
    if (doRayleighDamping != 2) 
      theEle = new ZeroLengthVG_HG(idata[0], ndm, idata[1], idata[2], idata[3], x, y, numMats, theMats, dirs, tol, doRayleighDamping);
    else
      theEle = new ZeroLengthVG_HG(idata[0], ndm, idata[1], idata[2], idata[3], x, y, numMats, theMats, theDampMats, dirs, tol, doRayleighDamping);
...
//  and storing the tags of the ZeroLengthVG_HG end nodes.



//  Construct element with multiple unidirectional materials
ZeroLengthVG_HG::ZeroLengthVG_HG(int tag,
				 int dim,
				 ...
				 double tolerance,
				 int doRayleigh)
  :Element(tag,ELE_TAG_ZeroLengthVG_HG),     
   ...
   numMaterials1d(n1dMat), theMaterial1d(0), dir1d(0), t1d(0), d0(0), v0(0)
{
  ...
  tol = tolerance;
    ...
    if ( theMaterial1d == 0 || dir1d == 0 ) {
      opserr << "FATAL ZeroLengthVG_HG::ZeroLengthVG_HG - failed to create a 1d  material or direction array\n";
      exit(-1);
    }
    ...
    *dir1d = direction;
	...
	opserr << "FATAL ZeroLengthVG_HG::ZeroLengthVG_HG - failed to get a copy of material " <<theMat[i]->getTag() << endln;
	exit(-1);
      }
     }
...
//   to be invoked upon
ZeroLengthVG_HG::ZeroLengthVG_HG(void)
  :Element(0,ELE_TAG_ZeroLengthVG_HG),     
  connectedExternalNodes(2),
  ...
  dir1d(0), t1d(0), d0(0), v0(0)
{
  ...
  springActive = true;
    ...
    if (connectedExternalNodes.Size() != 2)
      opserr << "FATAL ZeroLengthVG_HG::ZeroLengthVG_HG - failed to create an ID of correct size\n";
...
int
ZeroLengthVG_HG::getNumExternalNodes(void) const
{
    return 2;
}
...
const ID &
ZeroLengthVG_HG::getExternalNodes(void) 
{
...
Node **
ZeroLengthVG_HG::getNodePtrs(void) 
{
  return theNodes;
}

int
ZeroLengthVG_HG::getNumDOF(void) 
{
    return numDOF;
}
...
//    with the ZeroLengthVG_HG element, we set matrix and vector pointers,
//    allocate space for t matrix and define it as the basic deformation-
//    displacement transformation matrix.
void
ZeroLengthVG_HG::setDomain(Domain *theDomain)
{
    ...
    numDOF = 2;
    theMatrix = &ZeroLengthVG_HGM6;
    theVector = &ZeroLengthVG_HGV6;
    ...
    if ( theNodes[0] == 0 || theNodes[1] == 0 || node3Ptr == 0) {
      if (theNodes[0] == 0) 
        opserr << "WARNING ZeroLengthVG_HG::setDomain() - Nd1: " << Nd1 << " does not exist in ";
      else
        opserr << "WARNING ZeroLengthVG_HG::setDomain() - Nd2: " << Nd2 << " does not exist in ";

      opserr << "model for ZeroLengthVG_HG ele: " << this->getTag() << endln;
    ...
    int dofNd3 = node3Ptr->getNumberDOF();	

    // if differing dof at the ends - print a warning message
    if ( dofNd1 != dofNd2 || dofNd1 != dofNd3) {
      opserr << "WARNING ZeroLengthVG_HG::setDomain(): nodes " << Nd1 << " and " << Nd2 <<
	"have differing dof at ends for ZeroLengthVG_HG " << this->getTag() << endln;
      return;
    }	
    ...
    if (L > LENTOL*vm)
      opserr << "WARNING ZeroLengthVG_HG::setDomain(): Element " << this->getTag() << " has L= " << L << 
	...
	elemType  = D2N6;
      ...
      opserr << "WARNING ZeroLengthVG_HG::setDomain cannot handle " << dimension << 
...
int
ZeroLengthVG_HG::commitState()
{
    int code=0;

    // call element commitState to do any base class stuff
    if ((code = this->Element::commitState()) != 0) {
      opserr << "ZeroLengthVG_HG::commitState () - failed in base class";
    }    
...
int
ZeroLengthVG_HG::revertToLastCommit()
{
    int code=0;
...
ZeroLengthVG_HG::update(void)
{
    ...
    // determine if spring active or not
    const Vector &disp2T = theNodes[1]->getTrialDisp();
    const Vector &disp3T = node3Ptr->getTrialDisp();
    double dvT = disp2T[1]-disp3T[1];

    if (dvT < tol) {
      springActive = true;

      const Vector &disp2C = theNodes[1]->getDisp();
      const Vector &disp3C = node3Ptr->getDisp();
      double dvC = disp2C[1]-disp3C[1];

      if (dvC > tol) {
	Vector disp2New(disp2T);
	disp2New[0] = 0.0;
	theNodes[1]->setTrialDisp(disp2New);
      }

    } else
      springActive = false;

    // ********* END NEW CODE for JASON  *****************
...
const Matrix &
ZeroLengthVG_HG::getTangentStiff(void)
{
    double E;
...
const Matrix &
ZeroLengthVG_HG::getInitialStiff(void)
{
    double E;
...
void 
ZeroLengthVG_HG::zeroLoad(void)
{
  // does nothing now
}

int 
ZeroLengthVG_HG::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  opserr << "ZeroLengthVG_HG::addLoad - load type unknown for truss with tag: " << this->getTag() << endln;
  
  return -1;
}

int 
ZeroLengthVG_HG::addInertiaLoadToUnbalance(const Vector &accel)
{
...
const Vector &
ZeroLengthVG_HG::getResistingForce()
{
  double force;
...
const Vector &
ZeroLengthVG_HG::getResistingForceIncInertia()
{	
...
int
ZeroLengthVG_HG::sendSelf(int commitTag, Channel &theChannel)
{
	int res = 0;
	...
	// ZeroLengthVG_HG packs its data into an ID and sends this to theChannel
	...
	if (res < 0) {
	  opserr << "ZeroLengthVG_HG::sendSelf -- failed to send ID data\n";
	  return res;
	}
	...
	if (res < 0) {
	  opserr <<  "ZeroLengthVG_HG::sendSelf -- failed to send transformation Matrix\n";
	  return res;
	}
	...
	if (numMaterials1d < 1)
	  ...
	  if (res < 0) {
	    opserr << " ZeroLengthVG_HG::sendSelf -- failed to send classTags ID\n";
	    return res;
	  }
	  ...
	  for (i = 0; i < numMaterials1d; i++) {
	    ...
	    if (res < 0) {
	      opserr << "ZeroLengthVG_HG::sendSelf -- failed to send Material1d " << i << endln;
...
int
ZeroLengthVG_HG::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  
  int dataTag = this->getDbTag();

  // ZeroLengthVG_HG creates an ID, receives the ID and then sets the 
  ...
  if (res < 0) {
    opserr << "ZeroLengthVG_HG::recvSelf -- failed to receive ID data\n";
  ...
  if (res < 0) {
    opserr << "ZeroLengthVG_HG::recvSelf -- failed to receive transformation Matrix\n";
  ...
  if (idData(3) < 1) {
	...
	theMaterial1d = 0;
      ...
      theMaterial1d = new UniaxialMaterial *[numMaterials1d];
      if (theMaterial1d == 0) {
	opserr << "ZeroLengthVG_HG::recvSelf -- failed to new Material1d array\n";
	return -1;
      }
	...
	delete dir1d;
      dir1d = new ID(numMaterials1d);
      if (dir1d == 0) {
	opserr << "ZeroLengthVG_HG::recvSelf -- failed to new dir ID\n";
    ...
    if (res < 0) {
      opserr << "ZeroLengthVG_HG::recvSelf -- failed to receive classTags ID\n";
      return res;
    }
	...
	theMaterial1d[i] = theBroker.getNewUniaxialMaterial(matClassTag);
      ...
      if (theMaterial1d[i] == 0) {
	opserr << "ZeroLengthVG_HG::recvSelf  -- failed to allocate new Material1d " << i << endln;
	return -1;
      }
      ...
      if (res < 0) {
	opserr << "ZeroLengthVG_HG::recvSelf  -- failed to receive new Material1d " << i << endln;
	return res;
      }
...
void
ZeroLengthVG_HG::Print(OPS_Stream &s, int flag)
{
    ...
    if (flag == OPS_PRINT_CURRENTSTATE) { // print everything
        ...
        s << " type: ZeroLengthVG_HG  iNode: " << connectedExternalNodes(0);
...
Response*
ZeroLengthVG_HG::setResponse(const char **argv, int argc, OPS_Stream &output)
{
    Response *theResponse = 0;

    output.tag("ElementOutput");
    output.attr("eleType","ZeroLengthVG_HG");
...
int 
ZeroLengthVG_HG::getResponse(int responseID, Information &eleInformation)
{
    const Vector& disp1 = theNodes[0]->getTrialDisp();
    const Vector& disp2 = theNodes[1]->getTrialDisp();
...
const Vector &
ZeroLengthVG_HG::getResistingForceSensitivity(int gradIndex)
{
...
int
ZeroLengthVG_HG::commitSensitivity(int gradIndex, int numGrads)
{
...
// for orientation
void
ZeroLengthVG_HG::setUp( int Nd1, int Nd2,
		   const Vector &x,
		   const Vector &yp )
{ 
    // ensure the connectedExternalNode ID is of correct size & set values
    if (connectedExternalNodes.Size() != 2)
      opserr << "FATAL ZeroLengthVG_HG::setUp - failed to create an ID of correct size\n";
	...
	opserr << "FATAL ZeroLengthVG_HG::setUp - incorrect dimension of orientation vectors\n";
    ...
    if (xn == 0 || yn == 0 || zn == 0) {
      opserr << "FATAL ZeroLengthVG_HG::setUp - invalid vectors to constructor\n";
    }
...
void
ZeroLengthVG_HG::checkDirection( ID &dir ) const
{
    for ( int i=0; i<dir.Size(); i++)
	if ( dir(i) < 0 || dir(i) > 5 ) {
	  opserr << "WARNING ZeroLengthVG_HG::checkDirection - incorrect direction " << dir(i) << " is set to 0\n";
	  dir(i) = 0;
	}
}
...
// uniaxial materials
void
ZeroLengthVG_HG::setTran1d( Etype elemType,
		       int   numMat )
{
    ...
    if (t1d == 0)
	opserr << "FATAL ZeroLengthVG_HG::setTran1d - can't allocate 1d transformation matrix\n";
...
// of node 1
double
ZeroLengthVG_HG::computeCurrentStrain1d( int mat,
...
void
ZeroLengthVG_HG::updateDir(const Vector& x, const Vector& y)
{
