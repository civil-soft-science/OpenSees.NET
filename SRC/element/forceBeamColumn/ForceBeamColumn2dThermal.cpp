...
// $Source: /usr/local/cvs/OpenSees/SRC/element/forceBeamColumn/ForceBeamColumn2dThermal.cpp,v $
...
Matrix ForceBeamColumn2dThermal::theMatrix(6,6);
...
ForceBeamColumn2dThermal::ForceBeamColumn2dThermal(): 
  Element(0,ELE_TAG_ForceBeamColumn2dThermal), connectedExternalNodes(2), 
  ...
  Ki(0), parameterID(0),counterTemperature(0),Vsth0(0)
{
  theNodes[0] = 0;  
  theNodes[1] = 0;
}
...
ForceBeamColumn2dThermal::ForceBeamColumn2dThermal (int tag, int nodeI, int nodeJ,
				      int numSec, SectionForceDeformation **sec,
				      BeamIntegration &bi,
				      CrdTransf &coordTransf, double massDensPerUnitLength,
				      int maxNumIters, double tolerance):
  Element(tag,ELE_TAG_ForceBeamColumn2dThermal), connectedExternalNodes(2),
  beamIntegr(0), numSections(0), sections(0), crdTransf(0),
  ...
  Ki(0), parameterID(0),counterTemperature(0),Vsth0(0)
{
  ...
  if (beamIntegr == 0) {
    opserr << "Error: ForceBeamColumn2dThermal::ForceBeamColumn2dThermal: could not create copy of beam integration object" << endln;
    exit(-1);
  }
  ...
  if (crdTransf == 0) {
    opserr << "Error: ForceBeamColumn2dThermal::ForceBeamColumn2dThermal: could not create copy of coordinate transformation object" << endln;
    exit(-1);
  }
...
int
ForceBeamColumn2dThermal::getNumExternalNodes(void) const
{
  return 2;
}

const ID &
ForceBeamColumn2dThermal::getExternalNodes(void) 
{
  return connectedExternalNodes;
}

Node **
ForceBeamColumn2dThermal::getNodePtrs()
{
  return theNodes;
}

int
ForceBeamColumn2dThermal::getNumDOF(void) 
{
  return NEGD;
}

void
ForceBeamColumn2dThermal::setDomain(Domain *theDomain)
{
  ...
  if (theDomain == 0) {
    ...
    opserr << "ForceBeamColumn2dThermal::setDomain:  theDomain = 0 ";
    exit(0); 
  }
  ...
  if (theNodes[0] == 0) {
    opserr << "ForceBeamColumn2dThermal::setDomain: Nd1: ";
    opserr << Nd1 << "does not exist in model\n";
    exit(0);
  }
  
  if (theNodes[1] == 0) {
    opserr << "ForceBeamColumn2dThermal::setDomain: Nd2: ";
  ...
  if ((dofNode1 != NND) || (dofNode2 != NND)) {
    opserr << "ForceBeamColumn2dThermal::setDomain(): Nd2 or Nd1 incorrect dof ";
    exit(0);
  }
   
  // initialize the transformation
  if (crdTransf->initialize(theNodes[0], theNodes[1])) {
    opserr << "ForceBeamColumn2dThermal::setDomain(): Error initializing coordinate transformation";  
    exit(0);
  }
  ...
  if (L == 0.0) {
    opserr << "ForceBeamColumn2dThermal::setDomain(): Zero element length:" << this->getTag();  
    exit(0);
  }
...
int
ForceBeamColumn2dThermal::commitState()
{
  int err = 0;
  int i = 0;
  ...
  if ((err = this->Element::commitState()) != 0) {
    opserr << "ForceBeamColumn2dThermal::commitState () - failed in base class";
  }    
...
int ForceBeamColumn2dThermal::revertToLastCommit()
{
  int err;
  int i = 0;
...
int ForceBeamColumn2dThermal::revertToStart()
{
...
const Matrix &
ForceBeamColumn2dThermal::getInitialStiff(void)
{
  ...
  */
  ...
  if (f.Solve(I, kvInit) < 0)
    opserr << "ForceBeamColumn2dThermal::getInitialStiff() -- could not invert flexibility\n";
  */
...
void
ForceBeamColumn2dThermal::computeReactions(double *p0)
{
  int type;
  double L = crdTransf->getInitialLength();
  ...
  for (int i = 0; i < numEleLoads; i++) {
    ...
    double q0Temperature[3] ;
	q0Temperature[0]=0; q0Temperature[1]=0; q0Temperature[2]=0; 
    ...
    for (int seci = 0; seci < numSections; seci++) {
      
      int order = sections[seci]->getOrder();
      const ID &code = sections[seci]->getType();
      
      double xi6 = 6.0*xi[seci];
      
      // Get section stress resultant 
      // FMk const Vector &s = theSections[seci]->getTemperatureStress(dataMix);
      ...
      const Vector &s = sections[seci]->getTemperatureStress(dataMixV);    //contribuited by ThermalElongation
	...
	counterTemperature=1;
	Se(0)=Se(0)-q0Temperature[0];
	Se(1)=Se(1)-q0Temperature[1];
	Se(2)=Se(2)-q0Temperature[2];
	}

  }
	//end of beam thermal action
    ...
    for (int seci = 0; seci < numSections; seci++) {
	  ...
	  for(int m = 0; m<ndm; m++){
			theIntCrds(m) =  theNode0Crds(m)+xi[seci]*(theNode1Crds(m)- theNode0Crds(m));
	  }
#ifdef _BDEBUG
	  opserr<<"DispBeamColumn2dThermal::addLoad at the ele "<<this->getTag()<< " section "<<seci<<" --theIntCrds"<<theIntCrds<<endln;
#endif
	  //NodalThermalActions attached with the wrapper would have been updated by pattern;
	 ThermalActionWrapper* theLoad= (ThermalActionWrapper*)eleLoads[i];
	  Vector dataMixV = theLoad->getIntData(theIntCrds);
...
void
ForceBeamColumn2dThermal::computeReactionSensitivity(double *dp0dh, int gradNumber)
{
  int type;
  double L = crdTransf->getInitialLength();
...
const Vector &
ForceBeamColumn2dThermal::getResistingForce(void)
{
  ...
  Vector p0Vec(p0, 3);
  p0Vec.Zero();
//Modified for thermal action-----UoE 2015
  //if( counterTemperature ==1){
	  //this->update();
	 // counterTemperature=2;
 //}else if( counterTemperature ==2){
...
void
ForceBeamColumn2dThermal::initializeSectionHistoryVariables (void)
{
  for (int i = 0; i < numSections; i++) {
 ...
 */
int
ForceBeamColumn2dThermal::update()
{
...
#ifdef _BDEBUG
		opserr<<"ForceBeamColumn2D::"<< this->getTag()<<"  dv "<<dv<<endln;
#endif
  if(counterTemperature==0){
  ...
  while (converged == false && numSubdivide <= maxSubdivisions) {
	...
	SsrSubdivide[i] = Ssr[i];
...
#ifdef _BDEBUG
	  opserr<<"ForceBasedBeam:kvTrial "<<kvTrial<<endln;
#endif
      ...
      dSe.addMatrixVector(0.0, kvTrial, dvTrial, 1.0);
	    #ifdef _BDEBUG
	  opserr<<"ForceBasedBeam:SeTrial "<<SeTrial<<  " dSe "<<dSe<<endln;
#endif
      SeTrial += dSe;  //Forces
	...
	for (j=0; j <numIters; j++) {
	  ...
	  vr.Zero();
#ifdef _BDEBUG
	  opserr<<"ForceBasedBeam:SeTrial "<<j<<"  "<<SeTrial<<  " dSe "<<dSe<<endln;
#endif
...
#ifdef _BDEBUG
		opserr<<"ForceColumn2D "<< this->getTag()<<" Trial "<<j<<" Sec "<<i <<" Trial force "<<Ss<<endln
			<<"Ssr "<<SsrSubdivide[i]<<endln;
#endif
	    // dSs = Ss - Ssr[i];
	    dSs = Ss;
...
#ifdef _BDEBUG
		opserr<<"ForceBeamColumn:: "<< this->getTag()<<" Sec "<<i<<" Trial vs "<<vsSubdivide[i]<<" dVs: "<<dvs<<endln;
			
#endif
	    if (sections[i]->setTrialSectionDeformation(vsSubdivide[i]) < 0) {
	      opserr << "ForceBeamColumn2dThermal::update() - section failed in setTrial\n";
	      return -1;
	    }
	...
	#ifdef _BDEBUG
		opserr<<"ele "<< this->getTag()<<" trial- "<<j<<" sec "<<i<<" dSs "<<dSs<<" dvs "<<dvs<<endln;
...
#ifdef _BDEBUG
		opserr<<"-----sec "<<i<<" -----VsSub-"<<vsSubdivide[i]<<" Vr(0) "<<vr(0)<<endln;
#endif
	  ...
	  if (f.Solve(I, kvTrial) < 0)
	    opserr << "ForceBeamColumn2dThermal::update() -- could not invert flexibility\n";
		
	  #ifdef _BDEBUG
		opserr<<"ForceBeamColumn:: "<< this->getTag()<<" final vin "<<vin<<"  dvTrial "<<dvTrial<<" Vr "<<vr<<endln;
...
#ifdef _BDEBUG
		opserr<<"ForceBeamColumn:: "<< this->getTag()<<" final dSe "<<dSe<<" final dv "<<dv<<endln;
...
void ForceBeamColumn2dThermal::getForceInterpolatMatrix(double xi, Matrix &b, const ID &code)
{
  b.Zero();
...
void ForceBeamColumn2dThermal::getDistrLoadInterpolatMatrix(double xi, Matrix &bp, const ID &code)
{
  bp.Zero();
...
const Matrix &
ForceBeamColumn2dThermal::getMass(void)
{ 
  theMatrix.Zero();
...
void 
ForceBeamColumn2dThermal::zeroLoad(void)
{
...
int
ForceBeamColumn2dThermal::addLoad(ElementalLoad *theLoad, double loadFactor)
{
...
void
ForceBeamColumn2dThermal::computeSectionForces(Vector &sp, int isec)
{
  int type;
  ...
  for (int i = 0; i < numEleLoads; i++) {
	...
	if (x <= a) {
	...
	else if(type == LOAD_TAG_Beam2dThermalAction||type == LOAD_TAG_ThermalActionWrapper||type == LOAD_TAG_NodalThermalAction){
	  ...
	  */
      ...
      opserr << "ForceBeamColumn2dThermal::addLoad -- load type unknown for element with tag: " <<
	this->getTag() << endln;
    }
  }
...
void
ForceBeamColumn2dThermal::computeSectionForceSensitivity(Vector &dspdh, int isec,
						  int gradNumber)
{
  int type;
  ...
  for (int i = 0; i < numEleLoads; i++) {
	...
	if (x <= a) {
      ...
      opserr << "ForceBeamColumn2dThermal::computeSectionForceSensitivity -- load type unknown for element with tag: " <<
	this->getTag() << endln;
    }
  }
}
...
int 
ForceBeamColumn2dThermal::addInertiaLoadToUnbalance(const Vector &accel)
{
...
const Vector &
ForceBeamColumn2dThermal::getResistingForceIncInertia()
{	
...
int
ForceBeamColumn2dThermal::sendSelf(int commitTag, Channel &theChannel)
{  
  ...
  if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
    opserr << "ForceBeamColumn2dThermal::sendSelf() - failed to send ID data\n";
    return -1;
  }    

  // send the coordinate transformation
  if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
    opserr << "ForceBeamColumn2dThermal::sendSelf() - failed to send crdTrans\n";
    return -1;
  }      

  // send the beam integration
  if (beamIntegr->sendSelf(commitTag, theChannel) < 0) {
    opserr << "ForceBeamColumn2dThermal::sendSelf() - failed to send beamIntegr\n";
    return -1;
  }      
  ...
  for (j = 0; j<numSections; j++) {
    if (sections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "ForceBeamColumn2dThermal::sendSelf() - section " << 
  ...
  if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
    opserr << "ForceBeamColumn2dThermal::sendSelf() - failed to send Vector data\n";
     return -1;
  }    
...
int
ForceBeamColumn2dThermal::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  //
  ...
  if (theChannel.recvID(dbTag, commitTag, idData) < 0)  {
    opserr << "ForceBeamColumn2dThermal::recvSelf() - failed to recv ID data\n";
    return -1;
  }    
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
      ...
      if (crdTransf == 0) {
	opserr << "ForceBeamColumn2dThermal::recvSelf() - failed to obtain a CrdTrans object with classTag" <<
  ...
  {
     opserr << "ForceBeamColumn2dThermal::sendSelf() - failed to recv crdTranf\n";
  ...
  if (beamIntegr == 0 || beamIntegr->getClassTag() != beamIntegrClassTag) {
      ...
      if (beamIntegr == 0) {
	opserr << "ForceBeamColumn2dThermal::recvSelf() - failed to obtain the beam integration object with classTag" <<
  ...
  {
     opserr << "ForceBeamColumn2dThermal::sendSelf() - failed to recv beam integration\n";
  ...
  if (numSections != idData(3)) {
    ...
    if (vscommit == 0) {
      opserr << "ForceBeamColumn2dThermal::recvSelf -- failed to allocate vscommit array\n";
    ...
    if (fs == 0) {
      opserr << "ForceBeamColumn2dThermal::recvSelf -- failed to allocate fs array\n";
      return -1;
    }
    ...
    if (vs == 0) {
      opserr << "ForceBeamColumn2dThermal::recvSelf -- failed to allocate vs array\n";
      return -1;
    }
    ...
    if (Ssr == 0) {
      opserr << "ForceBeamColumn2dThermal::recvSelf -- failed to allocate Ssr array\n";
      return -1;
    }
    ...
    if (sections == 0) {
      opserr << "ForceBeamColumn2dThermal::recvSelf() - " << 
	...
	opserr << "ForceBeamColumn2dThermal::recvSelf() - section " << 
	...
	sections[i] = theBroker.getNewSection(sectClassTag);
	if (sections[i] == 0) {
	  opserr << "ForceBeamColumn2dThermal::recvSelf() - Broker could not create Section of class type" <<
	    sectClassTag << endln;
	  return -1;
	}
      }
...
int
ForceBeamColumn2dThermal::getInitialFlexibility(Matrix &fe)
{
  fe.Zero();
...
int
ForceBeamColumn2dThermal::getInitialDeformations(Vector &v0)
{
  v0.Zero();
  if (numEleLoads < 1)
    return 0;
...
void ForceBeamColumn2dThermal::compSectionDisplacements(Vector sectionCoords[], Vector sectionDispls[]) const
{
   // get basic displacements and increments
...
void
ForceBeamColumn2dThermal::Print(OPS_Stream &s, int flag)
{
  if (flag == 2) {

    s << "#ForceBeamColumn2dThermal\n";
...
OPS_Stream &operator<<(OPS_Stream &s, ForceBeamColumn2dThermal &E)
{
  E.Print(s);
  return s;
}
...
ForceBeamColumn2dThermal::setSectionPointers(int numSec, SectionForceDeformation **secPtrs)
{
  ...
  if (secPtrs == 0) {
    opserr << "Error: ForceBeamColumn2dThermal::setSectionPointers -- invalid section pointer";
  }	  
  
  sections = new SectionForceDeformation *[numSections];
  if (sections == 0) {
    opserr << "Error: ForceBeamColumn2dThermal::setSectionPointers -- could not allocate section pointers";
  }  
  
  for (int i = 0; i < numSections; i++) {
    
    if (secPtrs[i] == 0) {
      opserr << "Error: ForceBeamColumn2dThermal::setSectionPointers -- null section pointer " << i << endln;
    }
    
    sections[i] = secPtrs[i]->getCopy();

    if (sections[i] == 0) {
      opserr << "Error: ForceBeamColumn2dThermal::setSectionPointers -- could not create copy of section " << i << endln;
    }
  }
  ...
  fs  = new Matrix [numSections];
  if (fs == 0) {
    opserr << "ForceBeamColumn2dThermal::setSectionPointers -- failed to allocate fs array";
  }
  
  vs = new Vector [numSections];
  if (vs == 0) {
    opserr << "ForceBeamColumn2dThermal::setSectionPointers -- failed to allocate vs array";
  }
  
  Ssr  = new Vector [numSections];
  if (Ssr == 0) {
    opserr << "ForceBeamColumn2dThermal::setSectionPointers -- failed to allocate Ssr array";
  }
  
  vscommit = new Vector [numSections];
  if (vscommit == 0) {
    opserr << "ForceBeamColumn2dThermal::setSectionPointers -- failed to allocate vscommit array";   
  }
...
int 
ForceBeamColumn2dThermal::getResponseSensitivity(int responseID, int gradNumber,
					  Information &eleInfo)
{
  // Basic deformation sensitivity
  if (responseID == 3) {  
...
int
ForceBeamColumn2dThermal::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
...
int
ForceBeamColumn2dThermal::updateParameter (int parameterID, Information &info)
{
  if (parameterID == 1) {
    rho = info.theDouble;
    return 0;
...
int
ForceBeamColumn2dThermal::activateParameter(int passedParameterID)
{
...
const Matrix&
ForceBeamColumn2dThermal::getKiSensitivity(int gradNumber)
{
  theMatrix.Zero();
  return theMatrix;
}
...
const Matrix&
ForceBeamColumn2dThermal::getMassSensitivity(int gradNumber)
{
  theMatrix.Zero();
  return theMatrix;
}
...
const Vector&
ForceBeamColumn2dThermal::getResistingForceSensitivity(int gradNumber)
{
  static Vector dqdh(3);
...
int
ForceBeamColumn2dThermal::commitSensitivity(int gradNumber, int numGrads)
{
  int err = 0;
...
const Vector &
ForceBeamColumn2dThermal::computedqdh(int gradNumber)
{
...
const Matrix&
ForceBeamColumn2dThermal::computedfedh(int gradNumber)
{
  static Matrix dfedh(3,3);
