...
void* OPS_Trilinwp()
{
 ...
 // }
  ...
  numDatatot=numDatatot+numData;
     if (numDatatot != 21 ) { 
    opserr << "Invalid Args want: uniaxialMaterial Trilinwp tag? Fcrp? dcrp? Fyp? dyp? Fup? dup? Fcrn? dcrn? Fyn? dyn? Fun? dun? px? py? d1? d2? beta? Pt? Pb? itype?  ";            
    return 0;        
  }
  ...
  UniaxialMaterial* mat= new Trilinwp(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], dData[9], dData[10], dData[11], dData[12], dData[13], dData[14], dData[15], dData[16], dData[17], dData[18], iData[1]);       


  if (mat == 0) {
    opserr << "WARNING could not create uniaxialMaterial of type Trilinwp\n";
    return 0;
  }
...
Trilinwp::Trilinwp(int tag,
	...
	UniaxialMaterial(tag, MAT_TAG_Trilinwp),
...
Trilinwp::Trilinwp():
UniaxialMaterial(0, MAT_TAG_Trilinwp),
...
int
Trilinwp::setTrialStrain(double strain, double strainRate)
{
...
double
Trilinwp::getStrain(void)
{
	return Tstrain;
}

double
Trilinwp::getStress(void)
{
	return Tstress;
}
...
double
Trilinwp::getTangent(void)
{
  return Ttangent;
}

void
Trilinwp::positiveIncrement(double dStrain)
{
...
void
Trilinwp::negativeIncrement(double dStrain)
{
	double kn = pow(CrotMin / rot1n, beta);
	kn = (kn < 1.0) ? 1.0 : 1.0 / kn;
	double kp = pow(CrotMax / rot1p, beta);
...
int
Trilinwp::revertToLastCommit(void)
{
	TrotMax = CrotMax;
	TrotMin = CrotMin;
	TrotPu = CrotPu;
...
UniaxialMaterial*
Trilinwp::getCopy(void)
{
	Trilinwp *theCopy = new Trilinwp (this->getTag(),
...
int
Trilinwp::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) 
    opserr << "Trilinwp::sendSelf() - failed to send data\n";
...
int
Trilinwp::recvSelf(int commitTag, Channel &theChannel, 
			FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
      opserr << "Trilinwp::recvSelf() - failed to receive data\n";
      return res;
  }
  else {
...
void
Trilinwp::Print(OPS_Stream &s, int flag)
{
...
double
Trilinwp::posEnvlpStress(double strain)
{
	if (strain <= 0.0)
		return 0.0;
...
double
Trilinwp::negEnvlpStress(double strain)
{
	if (strain >= 0.0)
		return 0.0;
...
double
Trilinwp::posEnvlpTangent(double strain)
{
  if (strain < 0.0)
    return E1p*1.0e-9;
  else if (strain <= rot1p)
...
double
Trilinwp::negEnvlpTangent(double strain)
{
  if (strain > 0.0)
    return E1n*1.0e-9;
  else if (strain >= rot1n)
...
double
Trilinwp::posEnvlpRotlim(double strain)
{
...
double
Trilinwp::negEnvlpRotlim(double strain)
{
