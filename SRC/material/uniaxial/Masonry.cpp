...
// 
// Description: This file contains the class implementation for Masonry Material, a material model based 
// on Crisafulli's Stress-Strain Law originally developed to model cyclic axial behavior of masonry.Masonry 
...
{ 
  ...
  // 
  theMaterial = new Masonry(iData[0], dData[0], dData[1],dData[2], dData[3], dData[4], dData[5],dData[6], 
 ...
 *******************************************************************************/ 
Masonry::Masonry(int tag, double _Fm,     double _Ft,  
                   ...
                   int _IENV): 
  UniaxialMaterial(tag, MAT_TAG_Masonry), Fm(_Fm), Ft(_Ft),  
 ...
 ****************************************************************************/ 
int 
Masonry::setTrialStrain(double strain, double strainRate) 
{ 
  double  A1 = Emo*Um/Fm; 
  double  A2 = 1 - A1*Um/Uult; 
  double  Ko = Area1*Emo/Length; 
 ...
 ****************************************************************************/ 
 double 
 Masonry::getStrain(void) 
 { 
   return D; 
 } 
  
 double 
 Masonry::getStress(void) 
 { 
   return F; 
 } 
  
  double 
 Masonry::getTangent(void) 
 { 
   return K; 
 } 
 
  double 
Masonry::getInitialTangent(void) 
{ 
  //return Ko; 
  return Emo;
} 
...
return 0; 
 ...
 int 
 Masonry::revertToLastCommit(void) 
 { 
   D      = cD; 
   F      = cF; 
   K      = cK; 
...
UniaxialMaterial* 
Masonry::getCopy(void) 
{ 
  Masonry *theCopy = new Masonry(this->getTag(), Fm, Ft, Um, Uult, Ucl,  
 ...
 ****************************************************************************/ 
 //1a. 
 int 
 Masonry::sendSelf(int commitTag, Channel &theChannel) 
 { 
   ...
   { 
     opserr << "Masonry::sendSelf() - failed to sendSelf\n"; 
     return -1; 
     }  
return 0;    
 } 
 //1b. 
 int 
 Masonry::recvSelf(int commitTag, Channel &theChannel,  
                         FEM_ObjectBroker &theBroker) 
 { 
   static Vector data(51); 
   if (theChannel.recvVector(this->getDbTag(), commitTag, data)<0) 
   { 
     opserr << "Masonry::recvSelf() - failed to recvSelf\n"; 
     return -1; 
   } 
...
return 0; 
  ...
  //2. 
  void 
  Masonry::Print(OPS_Stream &s, int flag) 
  { 
    s << "Masonry:: tag: " << this->getTag() << endln; 
//    s << "Masonry::(deformation, force, tangent) " << D << " " << F << " " << K << endln; 
...
//1. 
void 
Masonry::Comp_Envlp (double Uenv, double Um, double Fm, double Emo, double Uult, int IENV, double 
&Senv, double &Eenv ) 
{ 
...
//2. 
void 
Masonry::Unload_Reload( double U, double U1, double  U2, double S1, double S2, double E1, double E2,
double &S, double &Et ) 
{ 
...
//3. 
void 
Masonry::Plastic_Strain( double Uun, double Sun, double Um, double Fm, double Emo, double Ft, double 
...
//4. 
void 
Masonry::Stress_Tangent(double U, double DeltaU, double cU, double cS, double cEt,  
    ...
    int &RuleNo, int &InnerCycleNo, int &IVIR ) 
{ 
