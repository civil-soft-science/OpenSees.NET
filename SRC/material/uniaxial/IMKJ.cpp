...
int mode(int kon, int j) {
	return kon - j * int(kon / j);
}
...
IMKJ::setTrialStrain(double trialStrain, double strainRate)
{
	...
	sigmax = sigmaxP;
	sigmin = sigminP;
	epspl = epsplP;	//strain of the point where the unloading branch enters the opposite stress region
	epss0 = epss0P;	//strain of the asymptotes intersection
	sigs0 = sigs0P;    //stress of the asymptotes intersection
	epsr = epssrP;		//strain of the first asymptote start point
	sigr = sigsrP;		//stress of the first asymptote start point
	kon = konP;		//branch identifier
	b2 = b2P;			//relative slope of the second asymptote (with relation to the first slope)

	//branches numbering:
	//kon = 1:	positive loading till yield and cap midpoint (starts form opposite unloading point)
	//kon = 2:	negative loading till yield and cap midpoint (starts form opposite unloading point)
	//kon = 3:	positive loading till cap and post-cap midpoint 
	//kon = 4:	negative loading till cap and post-cap midpoint 
	//kon = 5:	pinched unloading from negative branch passing through the zero stress point
	//kon = 6:	pinched unloading from positive branch passing through the zero stress point
	//kon = 7:	pinched positive loading
	//kon = 8:	pinched negative loading
	//kon = 9:	positive descending loading
	//kon =10:	negative descending loading
	...
	if (kon == 0) {
		epsmax = epsyP;
		epsmin = -epsyN;
		if (fabs(eps) < 0.5 * std::min(epsyP, epsyN))
		{
			e = E0;
			sig = e * eps;
			return 0;
		...
		else if(epsP > 0) // positive loading
			kon = 1;
			//epss0 = epsmax;
			//sigs0 = FydP;
			//double e1 = (sigs0 - sigr) / (epss0 - epsr);
			//b2 = bP * E0 / e1;
		else // negative loading
			kon = 2;
	}
	//manage unloading from inelastic strain
	if ((mode(kon, 2) == 1 && deps < 0) || (mode(kon, 2) == 0 && deps > 0))
	{
		epsr = epsP;
		sigr = sigP;
		epspl = epsP - sigP / E0;
		...
		if (mode(kon, 2) == 0)
		{
			//unloading from negative
			if (epsP < epsmin)
			{
				epsmin = epsP;
				sigmin = sigP;
			}
			if (kon == 2 || cyclicFlagP == 1)			//bilinear
			{
				kon = 1;
				//epss0 = (FydP - EshP * epsyP - sigr + E0 * epsr) / (E0 - EshP);
				//sigs0 = FydP + EshP * (epss0 - epsyP);
				//double e1 = (sigs0 - sigr) / (epss0 - epsr);
				//b2 = bP * E0 / e1;
			}
			else {
			...
			//unloading from positive
			if (epsP > epsmax)
			{
				epsmax = epsP;
				sigmax = sigP;
			}
			if (kon == 1 || cyclicFlagN == 1)
			{
				kon = 2;
				epss0 = (-FydN + EshN * epsyN - sigr + E0 * epsr) / (E0 - EshN);
				sigs0 = -FydN + EshN * (epss0 + epsyN);
				double e1 = (sigs0 - sigr) / (epss0 - epsr);
				b2 = bN * E0 / e1;
				R = R0 * 2;
			}
	...
	if (kon == 5) {
		...
		else
		{
			...
			double Dr = epsr;
			double Fr = sigr;
			double D0 = (Fhm - Fr) / E0 + Dr;
			double DN = (eps - Dr) / (Dr - D0);
			double DNt = (rhoD * epsmax - Dr) / (D0 - Dr);
			double F0 = E0 * (D0 - Dr) + Fr;
			double FNt = (rhoF * sigmax - Fr) / (F0 - Fr);
			if (eps > rhoD * epsmax)
			{
				sig = Fhm / Dhm * (1 - rhoF) / (1 - rhoD) * (eps - Dhm) + Fhm;
				e = Fhm / Dhm * (1 - rhoF) / (1 - rhoD);
			}
			else {
				sig = FNt * DN / DNt * pow((1 + pow(fabs(DNt), 0.66667)) / (1 + pow(fabs(DN), 0.66667)), 1.5) * (Fr - sigmax) + Fr;
				e = FNt / DNt * pow((1 + pow(fabs(DNt), 0.66667)) / (1 + pow(fabs(DN), 0.66667)), 1.5) * (Fr - sigmax);
				e /= (Dr - D0);
			}
		}
	}
	...
	if (kon == 6) {
		if (eps < epsmin)
		{
			...
			double D0 = (Fhm - Fr) / E0 + Dr;
			double DN = (eps - Dr) / (Dr - D0);
			double DNt = (rhoD * Dhm - Dr) / (D0 - Dr);
			double F0 = E0 * (D0 - Dr) + Fr;
			double FNt = (rhoF * Fhm - Fr) / (F0 - Fr);
			if (eps < rhoD * epsmin)
			{
				sig = Fhm / Dhm * (1 - rhoF) / (1 - rhoD) * (eps - Dhm) + Fhm;
				e = Fhm / Dhm * (1 - rhoF) / (1 - rhoD);
				...
				sig = FNt * DN / DNt * pow((1 + pow(fabs(DNt), 0.66667)) / (1 + pow(fabs(DN), 0.66667)), 1.5) * (Fr - sigmin) + Fr;
				e = FNt / DNt * pow((1 + pow(fabs(DNt), 0.66667)) / (1 + pow(fabs(DN), 0.66667)), 1.5) * (Fr - sigmin);
				e /= (Dr - D0);
			}
		}
		//double rotrel = epspl;
...
{
	if ((mode(konP, 2) == 1 && sigP < 0) || (mode(konP, 2) == 0 && sigP > 0))
	{
