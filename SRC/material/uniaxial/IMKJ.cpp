...
IMKJ::setTrialStrain(double trialStrain, double strainRate)
{
	...
	if (deps > 0 && sig > Fyp)
	{
		sig = Fyp + (eps - epsyP) * bP * E0;
		e = bP * E0;
	}
	else if (deps < 0 && sig < -Fyn)
	{
		sig = -Fyn + (eps + epsyN) * bN * E0;
		e = bN * E0;
	}
...
/*int old_setTrialStrain(double trialStrain, double strainRate)
{
	if (stat == 0) {
		epsmax = epsyP;
		epsmin = -epsyN;
	...
	if ((stat % 2 == 1 && deps < 0) || (stat % 2 == 0 && deps > 0)) // unloading
	{
		epsr = epsP;
		sigr = sigP;
		epspl = epsP - sigP / E0;
		...
		if (stat % 2 == 1)
		{
			...
			if (cyclicFlagN == 1) // Bilinear
			{
				stat = 2;
				double df = std::max(sigP - FydP, 0.0);
				sigs0 = -FydN + df;
				epss0 = epspl + sigs0 / E0;
			}
			else if (cyclicFlagN == 2) // Peak-Oriented
			{
				stat = 2;
				epss0 = epsmin;
				sigs0 = sigmin;
		...
		else
		{
			...
			{
				stat = 1;
				double df = std::min(sigP + FydN, 0.0);
				sigs0 = FydP + df;
				epss0 = epspl + sigs0 / E0;
			}
			else if (cyclicFlagN == 2) // Peak-Oriented
...
void IMKJ::updateDamage()
{
	if (((statP == 1 || statP == 3) && sig < 0) || ((statP == 2 || statP == 4) && sig > 0))
	{
		...
		double& resFac = (statP == 2 || statP == 4) ? resFacP : resFacN;
		//double& Fyd = FydP;
		ExcurEnergy += dE;
