...
double ResilienceLow::getTangent(){
	switch (mode){
		case 1:{tangent=Ke;}// mode1曲线1,3
			break; 
		case 2:{tangent=(Pmax-PY)/(DPmax-DY);}	// 曲线1,2
			break;
		case 3:{tangent=(stressRFMode2+PY)/(strainRFMode2+DY);}	// 曲线3,5  mode3
			break;
		case 4:{tangent=(Pmax-PY)/(DPmax-DY);}	// mode4，曲线3,4
			break;
		case 5:
			{tangent=(stressRFMode4-PY)/(strainRFMode4-DY);}		// mode5,曲线4,5
			break;
		case 6:
			{	if(stress>=0.55*Pmax){        
               tangent=-Kd;}
			...
			break;
		case 7:{tangent=Kui;}// mode 7 曲线7,8
			break;
		case 8:{tangent=Kri;}
			break;
		case 9:{tangent=Kui;}
